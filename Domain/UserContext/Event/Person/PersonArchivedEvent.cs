...
namespace SCAS.Domain.UserContext
{
    ...
    public class PersonArchivedEvent
        : DomainEventBase<DomainEventValue, PersonArchivedEventData>
    {
        [NotNull] private Person person;

        public override string Message => string.Format("Person {0} archived.", person.Info.Name);

        internal PersonArchivedEvent(Person targetPerson, IExtractor extractor = null)
            : base((uint)SCASEvent.PersonArchived, (uint)SCASEventType.Model, (uint)SCASEventLevel.Common, (uint)SCASEventPriority.Common, new PersonArchivedEventData(targetPerson), extractor)
        {
            person = targetPerson;
        }
