...
namespace SSUtils
{
	namespace Encryption
	{
		namespace RSA
		{
			...
			*	\说明: 		使用RSA公钥对信息进行加密
			*/
			SSUtils_API_DECLSPEC std::string encrypt(const std::string &publicKey, const std::string &origin, const std::string &seed = Data::toString(Data::generateRandomBlock()));

			struct SSUtils_API_DECLSPEC encrypter
			{
				...
				std::string operator()(const std::string &origin) const;
			...
			*	\说明: 		使用RSA私钥对密文进行解密
			*/
			SSUtils_API_DECLSPEC std::string decrypt(const std::string &privateKey, const std::string &cipher);

			struct SSUtils_API_DECLSPEC decrypter
			{
				...
				std::string operator()(const std::string &cipher) const;
			...
			*	\说明: 		使用RSA私钥签名文件
			*/
			SSUtils_API_DECLSPEC std::string sign(const std::string &privateKey, const std::string &msg);

			struct SSUtils_API_DECLSPEC signer
			{
				signer(void) = default;
			...
			*	\说明: 		使用RSA公钥验证文件签名
			*/
			SSUtils_API_DECLSPEC const bool verify(const std::string &publicKey, const std::string &msg, const std::string &signature);

			struct SSUtils_API_DECLSPEC verifier
			{
				...
				const bool operator()(const std::string &msg, const std::string &signature) const;
